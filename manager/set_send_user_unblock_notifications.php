...
if ($sendNotifications)
    $query = "UPDATE Setting SET SendUserUnblockNotification = 1";
else
    $query = "UPDATE Setting SET SendUserUnblockNotification = 0";
$res = mysqli_query($conn,$query);
if ( ! $res)
    die (json_encode(["error" => "cmd failed : " . $query]));
echo json_encode(["sendUserUnblockNotification" => $sendNotifications]);
