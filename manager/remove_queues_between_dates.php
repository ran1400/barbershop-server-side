...
else
{
    ...
    $query = "SELECT UserMail,Time FROM ReservedQueue WHERE Time > '$timeStamp' AND Time BETWEEN ? AND ?";
    ...
    $reservedQueuesDeleted = runExecQuery($conn,$query,[$firstDate,$secondDate]);
    if ($reservedQueuesDeleted === null)
       die(json_encode(["error" => "cmd failed : " . $query])); 
    $query = "DELETE FROM EmptyQueue WHERE Time > '$timeStamp' AND Time BETWEEN ? AND ?";
    $emptyQueuesDeleted = runExecQuery($conn,$query,[$firstDate,$secondDate]);
    if ($emptyQueuesDeleted === null)
        die(json_encode(["error" => "cmd failed : " . $query]));
    ...
    $conn->commit();
    echo json_encode(["reservedQueuesDeleted" => $reservedQueuesDeleted,"emptyQueuesDeleted" => $emptyQueuesDeleted]);
}
